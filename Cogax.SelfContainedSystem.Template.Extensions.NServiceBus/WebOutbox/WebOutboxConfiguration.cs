...
public class WebOutboxConfiguration
{
	...
	private readonly UnicastRoutingTable _unicastRoutingTable = new();

	private readonly EndpointConfiguration _outboxEndpointConfiguration;
	private readonly List<Action<EndpointConfiguration>> _outboxEndpointConfigurationActions = new();
	...
	public async Task<WebOutbox> StartOutbox()
	{
		...
		WebOutbox = new WebOutbox(outboxEndpoint, forwarderEndpoint, destinationEndpoint);

        return WebOutbox;
    }

    public async Task StopOutbox()
    {
        if(WebOutbox != null)
